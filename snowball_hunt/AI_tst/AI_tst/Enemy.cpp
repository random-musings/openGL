...
Enemy::Enemy(void):circle(0),
					maxSnowBallMakeStage(5),
					maxSnowBallThrowStage(5),
					currentState(IDLE),
					angleLegSwing(10),
					angleArmSwing(10),
					incr(1),
...
void 	Enemy::UpdateCurrentState()
{
	...
	//check to see if NPC is in danger (vehicle is facing us and withing range)
	bool inDanger = false ;///IsVehicleInRange();
	...
	if(inDanger)
	{
		if(currentState==CARRY_SNOWBALL )
		{
	...
	}else
	{
		...
		if(currentState==CARRY_SNOWBALL)
		{	//is destination set?
			//if not then find plot path-to next gift to visit
			//
			//setVelocity(Point(1,0,1));
