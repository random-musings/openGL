...
Point giftOrigin1(-43,-45,38); //behind the fractal tree
Point giftOrigin2(45,-45,-20); //under the snow drift
Point giftOrigin3(0,-45,-20); //in the air rotating
Point giftOrigin4(30,-45,40); //on top of thesnowman
Point giftOrigin5(-43,-45,-43); //corner behind bezier drift
...
update()
{
	...
	//tmpObj.setVelocity(tmpObj.getVelocity() + tmpObj.getVelocity());
	//if (processCollision(tmpObj)) //process collisions and damage taken
	//{
	//	vehicle.setVelocity(Point(0,0,0));
	//	vehicle.setSpeed(0);
 	//	currentOrigin.x =tmpObj.getOrigin().x;
	//	currentOrigin.z =tmpObj.getOrigin().z;
	//	}
	...
	if(activeBeam) //do not respond to events when beam is activated-drop vehicle velocity to zero
	{
		...
		//adjustVelocityMagnitude(vehicle.getSpeed());
		return; 
	}
...
//		adjustVelocityMagnitude(vehicle.getSpeed()));
		currentOrigin = currentOrigin + vehicle.getVelocity();
	}
    else
	{
		vehicle.setSpeed(0);
	...
	currentOrigin.x += vehicle.getVelocity().x;
	currentOrigin.z += vehicle.getVelocity().y;
...
void keyBoard( unsigned char key, int x, int y )
{
    ...
    switch( key )  {
		case ' ':
				...
				//vehicle.setVelocity(adjustVelocityMagnitude(vehicle.getSpeed(),);
				break;
		case 0:
				//vehicle.adjustVelocityMagnitude(vehicle.setSpeed(-speed));
				vehicle.setSpeed(-vehicle.getSpeed());
				//speed =-speed;
				break;
...
//							-50,10); //min max z
   ...
   glHint(GL_PERSPECTIVE_CORRECTION_HINT, GL_NICEST);
	...
	currMap.createMap(Point(-45,-45,-45), 
					Point(45,45,45), 
					npc, 
					gifts, 
					giftCount, 
	...
	controller = PathController(&currMap,&npc);
