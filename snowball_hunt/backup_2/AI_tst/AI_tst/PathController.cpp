...
void PathController::findHidingPlace()
{
	...
	Point currentNodeIx = currMap->getNode(character->getOrigin())->getNodeIndex();
	Point enemyIx =currMap->getNode(opponent->getOrigin())->getNodeIndex();
...
void PathController::setCharacterStates()
{
	...
	MapNode *tmp;
	if((tmp=currMap->getNode(character->getOrigin())) !=0) {currentNodeIx	=tmp->getNodeIndex();}	
	if((tmp = currMap->getNode(opponent->getOrigin())) !=0){enemyIx			=tmp->getNodeIndex();}

	float tmpDistanceOpponent = distanceFromNode(currentNodeIx, enemyIx );
	MapNode *tmpNode =ObstacleExistsBetween(currentNodeIx,enemyIx);

	if(!tmpNode && tmpDistanceOpponent<opponent->getStrikeRange())
