...
void Obstacle::setFacingAngleFromVelocity(Point newVelocity)
{
	velocity= newVelocity;
	facingAngle=(int) facingAngle%360;
	velocity.x = abs(velocity.x)<0.001?0:velocity.x;
	velocity.z = abs(velocity.z)<0.001?0:velocity.z;
...
void Obstacle::moveObstacle()
{
	if(velocity!=Point(0,0,0))
	{
		...
		setSpaceDimensions(getSpaceDimensions()); //reset the bounding box according to the origin
	}
}


Point Obstacle::makeUnitVector(Point newPt)
{
